...
from ansys.solutions.simple_beam_bending.ui.pages import compute_page, context_page
...
def display_page(pathname, value):
    ...
    if triggered_id == "url":
        return context_page.layout(project.steps.context_step)
    if triggered_id == "navigation_tree":
        if value is None:
            page_layout = html.H1("Welcome!")
        elif value == "context_step":
            page_layout = context_page.layout(project.steps.context_step)
        elif value == "compute_step":
            page_layout = compute_page.layout(project.steps.compute_step)
        return page_layout
